...
export async function getTaxComplianceReport(startDateStr: string, endDateStr: string) {
  const session = await getSession();
  if (!session) return { success: false, error: "Unauthorized" };
  ...
  try {
    ...
    let grossSales = 0;
    let exemptSales = 0; // VAT 0%
    let taxableSales = 0; // Ex VAT for standard
    let vatPayable = 0;   // VAT amount

    for (const t of transactions) {
      const price = Number(t.total_price);
      const vatRate = Number(t.product.vat_rate || 16);
    ...
    console.error("Failed to load tax compliance report:", error);
    return { success: false, error: error.message || "Failed to load tax report" };
  }
}
